...
#    define ANIM_SIZE_BONGO     320   // number of bytes in array. If you change sprites, minimize for adequate firmware size. max is 1024
...
static void render_bongocat(void) {
    ...
    void animate_bongocat(void) {
        ...
        if (current_wpm <= MIN_WALK_SPEED) {
            oled_write_raw_P(idle[0], ANIM_SIZE_BONGO);

        }
        if (current_wpm > MIN_RUN_SPEED) {
            current_frame_bongo = (current_frame_bongo + 1) % 2;
            oled_write_raw_P(tap[current_frame_bongo], ANIM_SIZE_BONGO);
        }
    }
